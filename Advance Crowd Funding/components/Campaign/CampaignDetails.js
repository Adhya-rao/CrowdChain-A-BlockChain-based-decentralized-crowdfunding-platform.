...
export default function CampaignDetails({ campaignId }) {
  ...
  const { releaseMilestone, isLoading: releasing } = useReleaseMilestone();
  const { getRefund, isLoading: refunding } = useGetRefund();

  // Milestone data
  const { data: milestones, isLoading: milestonesLoading, error: milestonesError } = useContractRead({
  ...
  const { data: releasedAmount, isLoading: releasedLoading, error: releasedError } = useContractRead({
    address: CONTRACT_ADDRESS,
    abi: CROWDFUNDING_ABI,
    functionName: "campaignReleasedAmount",
    args: [campaignId],
    enabled: Boolean(campaignId && CONTRACT_ADDRESS),
    watch: true,
  });
  ...
  const canWithdraw =
    isCreator && timeLeft.expired && isSuccessful && !campaign.withdrawn && !hasMilestones;
  const canGetRefund =
          ...
          <nav className="flex space-x-8">
            {["overview", "milestones", "updates", "contributors"].map((tab) => (
              ...
              >
                ...
                {tab === "milestones" && milestones && milestones.length > 0 && (
            ...
            {milestones && milestones.length > 0 ? (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    Released: {releasedAmount ? formatEther(releasedAmount) : "0"} ETH
                  </div>
                </div>

                <div className="space-y-4">
                  {/* Debug milestone data - REMOVE AFTER FIXING */}
                  {/* {console.log("=== ALL MILESTONES DATA ===")}
                  {console.log("Milestones array:", milestones)}
                  {console.log("Milestones length:", milestones.length)}
                  {milestones.forEach((m, i) => console.log(`Milestone ${i}:`, m))}
                  {console.log("===========================")} */}
                  {milestones.map((milestone, index) => {
                    const canRelease =
                      ...
                      isSuccessful &&
                      !milestone.released &&
                      (index === 0 || milestones[index - 1].released) &&
                      (!milestone.releaseTime || Date.now() / 1000 >= Number(milestone.releaseTime));

                    // Debug logging - REMOVE AFTER FIXING
                    // console.log(`=== Milestone ${index} Debug ===`);
                    // console.log('isCreator:', isCreator);
                    // console.log('isSuccessful:', isSuccessful);
                    // console.log('milestone.released:', milestone.released);
                    // console.log('isFirstMilestone:', index === 0);
                    // console.log('prevMilestoneReleased:', index > 0 ? milestones[index - 1].released : 'N/A');
                    // console.log('releaseTime:', milestone.releaseTime);
                    // console.log('currentTime:', Date.now() / 1000);
                    // console.log('timeCheck:', !milestone.releaseTime || Date.now() / 1000 >= Number(milestone.releaseTime));
                    // console.log('canRelease:', canRelease);
                    // console.log('Full milestone object:', milestone);
                    // console.log('========================');
                        ...
                        <div className="flex items-start justify-between">
                              ...
                              {milestone.releaseTime > 0 && (
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-gray-600 dark:text-gray-400">
                                    {milestone.released
                                      ? "Released"
                                      : Date.now() / 1000 >= Number(milestone.releaseTime)
                                        ? "Ready to release"
                                        : `Locked until ${formatDate(milestone.releaseTime)}`
                                    }
                                  </span>
                                </div>
                              )}
                          ...
                          {canRelease && (
                            <button
                              ...
                              disabled={releasing}
                              className="px-4 py-2 bg-purple-500 hover:bg-purple-600 disabled:bg-gray-400 text-white text-sm font-medium rounded-lg transition-colors disabled:cursor-not-allowed"
                            >
                ...
                {isCreator && isSuccessful && milestones.every(m => m.released) && (
                  <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                    <p className="text-green-800 dark:text-green-200 text-sm">
                      All milestones have been completed and funds released!
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 dark:text-gray-400">
                  This campaign doesn't have milestone-based funding.
                </p>
              </div>
            )}
          </div>
