...
export const useContract = () => {
  ...
  const useCreateCampaignWithMilestones = () => {
    const { write, writeAsync, isLoading, isSuccess, error } = useContractWrite(
      {
        address: CONTRACT_ADDRESS,
        abi: CROWDFUNDING_ABI,
        ...
        onSuccess(data) {
          toast.success("Campaign with milestones created successfully!");
        },
        onError(error) {
          console.error("Contract write error:", error);
          toast.error(error?.message || "Transaction failed");
    ...
    return {
      createCampaignWithMilestones: write,
      createCampaignWithMilestonesAsync: writeAsync,
      isLoading,
      isSuccess,
      error,
    };
  ...
  const useUserRewards = (userAddress) => {
  return useContractRead({
    address: CONTRACT_ADDRESS,
    abi: CROWDFUNDING_ABI,
    functionName: "getUserRewards", // Make sure your smart contract has this
    args: [userAddress],
  ...
  const useTopContributors = (topN = 10) => {
    const { data: rawData, ...rest } = useContractRead({
      address: CONTRACT_ADDRESS,
      abi: CROWDFUNDING_ABI,
    ...
    // Transform the tuple array response to an array of contributor objects
    const processedData = rawData && Array.isArray(rawData) && rawData.length >= 2 && Array.isArray(rawData[0]) && Array.isArray(rawData[1])
      ? rawData[0].map((address, index) => ({
          wallet: address,
          totalContributed: rawData[1][index] || 0n,
        }))
      : [];
  ...
  const useReleaseMilestone = () => {
    ...
    return {
      releaseMilestone: write,
      releaseMilestoneAsync: writeAsync,
      isLoading,
      isSuccess,
      error,
    };
  ...
  return {
    ...
    useCreateCampaignWithMilestones,
